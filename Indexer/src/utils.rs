use std::{collections::{HashMap, HashSet}, fmt, fs::File, io::Write, path::Path};
use lol_html::{element, rewrite_str, text, HtmlRewriter, RewriteStrSettings, Settings};
use human_regex::{exactly, one_or_more, or, punctuation, whitespace, word_boundary,zero_or_more,any,text as htext};
...
impl fmt::Display for ExtractedData {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        writeln!(f, "  full_text: {:?} words",self.full_text)?;
        write!(f, "}}")
    }
}

pub fn extract_structured_data(text: String) -> ExtractedData {
    let mut title = None;
        ...
        RewriteStrSettings {
            element_content_handlers: vec![
                ...
                element!("meta[property]", |el| {
                    if let (Some(property), Some(content)) = (el.get_attribute("property"), el.get_attribute("content")) {
                    ...
                    if let (Some(name), Some(content)) = (el.get_attribute("name"), el.get_attribute("content")) {
                        ...
                        if !content.is_empty() {
                            match name.as_str() {
                                ...
                                "url" => canonical_url = Some(content),
                                _ => {}
                            }
                        }
                    }
    ...
    title = og_title.or(meta_title).or(page_title);
    description = og_description.or(meta_description);
    canonical_url = og_url.or(canonical_url);

   
    let page_text = brackets_pattern.replace_all(paragraphs.join(" ").as_str()," ").trim().to_string();
    ...
    let text:Vec<String>=clean_corpus(page_text).split(" ").map(|x| x.trim().to_string()).filter(|x| !x.is_empty()).collect();
